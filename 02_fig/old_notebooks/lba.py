...
def resp_pdf(t, ind, A, b, v, s):
    ...
    # # P(no finish) term
    # p_neg, _ = reduce(
    #     fn=lambda vi, tot, s: normcdf(-vi / s) * tot,
    #     sequences=v,
    #     outputs_info=pt.ones(1, dtype="float64"),
    #     non_sequences=s
    # )

    # # PDF for chosen accumulator and no others have finished
    # v_idx = pt.arange(v.shape[0])
    # i = pt.cast(ind, 'int64')

    # res, _ = scan(
    #     fn=lambda tj, ij, v_idx, A, b, v, s:
    #         tpdf(tj, A, b, v[ij], s)
    #         * reduce(
    #             fn=lambda vj, prod, t, A, b, s: (1 - tcdf(t, A, b, vj, s)) * prod,
    #             sequences=v[pt.neq(v_idx, ij)],
    #             outputs_info=pt.ones_like(tj),
    #             non_sequences=[tj, A, b, s]
    #         )[0],
    #     sequences=[t, i],
    #     non_sequences=[v_idx, A, b, v, s]
    # )

    # # normalize and zero-out negative times
    # pdf_raw = res / (1 - p_neg)
    # return pt.switch(pt.gt(t, 0), pdf_raw, 0)
...
class LBA():
    ...
    def log_prob(self, p):
        ...
        clipped = pt.clip(p, 10e-10, float('inf'))
        return pt.log(clipped)
