...
class BlockOneRT:
    ...
    def initialize(self, key: jr.PRNGKey, method="prior", w_scale: float = 1e-3) -> Tuple[dict, dict]:
        ...
        params = {
            ...
            "scale_rt": jr.normal(ks[1], ()) * 0.1 + 1.0,
        }
        return params, {}
    ...
    def distribution(self, params: dict, inputs: jnp.ndarray):
        """
        inputs: [..., 8] array
        returns a tfd.Independent distribution of Normal(mu_rt, scale_rt)
        """
        if inputs.ndim == 2:
        ...
        # 1) RT ~ Normal(mu_rt, scale_rt)
        mu_rt = jnp.einsum('...i,i->...', x_rt, params["weights_rt"])
        scale_rt = jnn.softplus(params["scale_rt"]) + 1e-6

        return tfd.Independent(
            tfd.Normal(loc=mu_rt, scale=scale_rt),
