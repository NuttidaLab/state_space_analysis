...
class CircEmissions(HMMEmissions):
    ...
    def distribution(self, params, state, inputs = None):
        return tfd.Independent(
            tfd.VonMises(params.weights[state] @ inputs, params.covs[state]),
...
class ParamsCircHMMTransitions(NamedTuple):
    transition_weights: Union[Float[Array, "state_dim state_dim input_dim"], ParameterProperties]
...
class CircHMMTransitions(HMMTransitions):
    ...
    def initialize(self, key=None, method="prior", transition_weights=None):
        
        if transition_weights is None:
            this_key, key = jr.split(key)
            transition_weights = 0.01 * jr.normal(this_key, (self.num_states, self.num_states, self.input_dim))

        # Package the results into dictionaries
        params = ParamsCircHMMTransitions(transition_weights=transition_weights)
        props = ParamsCircHMMTransitions(transition_weights=ParameterProperties())
        return params, props

    def distribution(self, params, state, inputs=None):
        logits = params.transition_weights[state] @ inputs
...
class CircHMM(HMM):
    def __init__(self,
        ...
        transition_component = CircHMMTransitions(num_states, input_dim, concentration=transition_matrix_concentration, stickiness=transition_matrix_stickiness)
        emission_component = CircEmissions(num_states, input_dim, emission_dim)
    ...
    def initialize(self,
        ...
        params["transitions"], props["transitions"] = self.transition_component.initialize(key2, transition_weights=transition_weights)
